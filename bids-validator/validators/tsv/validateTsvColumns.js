...
const loadSchemas = tsvs => {
  ...
  const getSchemaByType = {
    ...
    nirs: () => require('../json/schemas/nirs.json'),
  }
      ...
      if (value_present && !mergedDict.hasOwnProperty(key)) {
        tsvIssues.push(
          new Issue({
            code: 211,
            file: tsv.file,
            evidence: `${tsv.file.name} has value: ${tsv_req.value} in column "${tsv_req.header}", which means ${key} should be defined in one of the corresponding json files`,
          }),
        )
      }
    }
