 ...
 * @returns
 */
const loadSchemas = async tsvs => {
  const schemas = {}
  const getSchemaByType = {
    blood: async () => (await import('../json/schemas/pet_blood.json')).default,
  }
  const types = Array.from(new Set(tsvs.map(tsv => getTsvType(tsv.file))))
  await Promise.all(
    types.map(async type => {
      if (getSchemaByType.hasOwnProperty(type)) {
        schemas[type] = await getSchemaByType[type]()
      }
    }),
  )
  return schemas
 ...
 */
const validateTsvColumns = async function(tsvs, jsonContentsDict, headers) {
  const tsvIssues = []
  const schemas = await loadSchemas(tsvs)
  ...
  tsvs.map(tsv => {
    ...
    aslHeaders.forEach(aslHeader => {
      ...
      if (associatedAslContext > -1) {
        ...
        const filtered_control_rows = rows.filter(row =>
          control_filters.includes(row),
        )
